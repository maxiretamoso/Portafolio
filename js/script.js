...
document.addEventListener("DOMContentLoaded", function () {
  ...
  //actualizar el enlace y el punto activo
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  //cerrar al clickear en overlay
  overlay.addEventListener("click", closeMobileMenu);

  //cerrar al clickear en enlaces
  navLinks.forEach((link) => {
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  const GITHUB_USERNAME = (
    gitSectionEl?.getAttribute("data-username") || "maxiretamoso"
  ).trim();
  ...
  //obtener estadísticas de usuario de github
  async function fetchUserStats() {
    try {
      ...
      document.getElementById("totalRepos").textContent = formatNumber(
        user.public_repos
      );

      //alcular estrellas y forks recorriendo los repositorios
      const reposResponse = await fetch(
        `https://api.github.com/users/${GITHUB_USERNAME}/repos?per_page=100&type=owner&sort=updated&direction=desc`,
        {
          headers: {
            ...getGitHubHeaders(),
            Accept: "application/vnd.github+json",
  ...
  async function fetchRepos() {
    try {
      ...
      const response = await fetch(
        `https://api.github.com/users/${GITHUB_USERNAME}/repos?sort=updated&direction=desc&per_page=20&type=owner`,
        {
          headers: {
            ...getGitHubHeaders(),
            Accept: "application/vnd.github+json",
            ...
            return `
              <div class="repo-card">
                ...
                <div class="repo-stats">
                  <div class="repo-stat"><span>⭐</span><span>${formatNumber(
                ...
                <div class="repo-links">
                  <a href="${
                    repo.html_url
                  }" target="_blank" class="repo-link">Ver Repo</a>
                  ${
                    repo.homepage
                      ? `<a href="${repo.homepage}" target="_blank" class="repo-link">Ver Demo</a>`
                      : ""
                  }
                </div>
              </div>
      ...
      // Inicializar logica del carrusel GitHub
      initGithubCarousel();
      // Ajustar altura del contenedor al alto del slide activo
      setTimeout(() => {
        const container = document.querySelector(
          ".github-carousel .carousel-container"
        );
      ...
      if (track)
        track.innerHTML = `<div class=\"loading-repos\">No se pudieron cargar repositorios.`;
    }
  }
  ...
  function collectGithubDom() {
    ...
    githubIndicators = Array.from(
      document.querySelectorAll("#githubIndicators .indicator")
    );
  }
  ...
  function showGithubSlide(index) {
    ...
    if (githubIndicators[githubCurrentSlide])
      githubIndicators[githubCurrentSlide].classList.add("active");
  ...
  function nextGithubSlide() {
    showGithubSlide(githubCurrentSlide + 1);
  }
  function prevGithubSlide() {
    showGithubSlide(githubCurrentSlide - 1);
  }
  ...
  function initGithubCarousel() {
    ...
    document.addEventListener("keydown", (e) => {
      if (e.key === "ArrowLeft") {
        e.preventDefault();
        prevGithubSlide();
        // Pausar 5s tras interacción por teclado
        stopGithubAutoPlay();
        setTimeout(startGithubAutoPlay, 5000);
    ...
    function handleGithubGesture() {
      const delta = touchEndX - touchStartX;
      if (Math.abs(delta) < 40) return;
      if (delta < 0) nextGithubSlide();
      else prevGithubSlide();
    }
  ...
  async function buildLanguagesFromAPI(repos) {
    ...
    // Traer hasta 50 repositorios para cubrir todos los lenguajes usados
    const subset = repos.slice(0, 50);
    const aggregate = {};
    try {
      const languagesArrays = await Promise.all(
      ...
      languagesArrays.forEach((obj) => {
        Object.entries(obj).forEach(([lang, bytes]) => {
          aggregate[lang] =
            (aggregate[lang] || 0) + (typeof bytes === "number" ? bytes : 0);
        });
      });
      ...
      if (entries.length === 0 || totalBytes === 0) {
        grid.innerHTML =
          '<div class="loading-repos">Sin datos de lenguajes</div>';
        return;
      }
      grid.innerHTML = "";
      const colorMap = {
      ...
      if (grid)
        grid.innerHTML =
          '<div class="loading-repos">Error al cargar lenguajes</div>';
    }
  }
});
